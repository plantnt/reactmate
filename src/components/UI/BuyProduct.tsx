...
const BuyProduct = () => {
  const sampleProduct = { id: 1, url: 'https://via.placeholder.com/150', alt: 'Sample Product' };
  ...
  const isFavorite = () => {
    return favorites.some((fav) => fav.id === sampleProduct.id);
  };

  const addFavorite = () => {
    setFavorites((prevFavorites) => [...prevFavorites, sampleProduct]);
  };

  const removeFavorite = () => {
    setFavorites((prevFavorites) =>
      prevFavorites.filter((item) => item.id !== sampleProduct.id)
    );
  };
  ...
  return (
    <div
      id="container"
      className="fixed flex items-center justify-left right-0 bottom-[120px] h-40 w-[400px] border rounded-lg p-3 bg-white transition-all z-[99] overflow-hidden"
    >
      <IoMdArrowDropleft
        size={40}
        id="openArrow"
        className="hidden text-slate-600 text-xl hover:cursor-pointer z-[100]"
        onClick={handleShow}
      />
      <IoMdArrowDropright
        size={40}
        id="closeArrow"
        className="text-slate-600 text-xl hover:cursor-pointer z-[100]"
        onClick={handleHide}
      />
      <div id="actions" className="block">
        <h1 className="font-semibold text-xl">Acciones</h1>
        ...
        <div className="flex items-center justify-center mt-4 gap-2">
          {isFavorite() ? (
            <div className="flex items-center space-x-3">
              <FaHeart
                size={12}
                className="text-convenientOrange cursor-pointer"
                onClick={handleFavorite}
              />
              <p className="text-xs">Quitar de favoritos</p>
            </div>
